...
-- in the query.
--I thought that 'AS' doesn't change the actual name in the dataset just how its presented 
--so how can we reference it elsewhere if the name has
...
--is the extra info displayed on \d+ always  "Storage  | Stats target | Description" 

-- A list of the top 3 countries by total number of customers
SELECT country, COUNT(*) AS total_number_of_customers FROM customers GROUP BY country ORDER BY total_number_of_customers DESC LIMIT 3;

-- A list of the top 7 cities (anywhere) by total number of customers
SELECT city, COUNT(*) AS total_number_of_customers 
FROM customers 
GROUP BY city 
